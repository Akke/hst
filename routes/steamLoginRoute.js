...
module.exports = (app, steam) => {
    app.get("/api/user/login", steam.authenticate(), function(req, res) {
	    res.status(200);
	});
	 
	app.get("/api/user/login/verify", steam.verify(), function(req, res) {
	...
	app.get("/api/auth/logout", steam.enforceLogin("/"), function(req, res) {
	    req.logout();
	    res.status(200);
	});

	app.get("/api/auth/fetch", verifyToken, function(req, res) {
		res.send({ token: req.token });
	});

	app.get("/api/auth/end", function(req, res) {
