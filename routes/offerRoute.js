...
module.exports = (app, csrfProtection, jsonParser, io) => {
    ...
    app.get("/api/offer", [jsonParser, csrfProtection], async(req, res) => {
    	let limit = 10,
    		skip = 0,
    		mode = "sc",
    		region = "ALL",
		...
		const query = { 
			...
			price: { 
				$gte: minPrice, //todo: differ if offer is auction or not (buyout price is unaffected by this)
				$lte: (maxPrice != "1") ? maxPrice : 9999999
			}
		};

		if(item) {
			Object.assign(query, { "item.value": parseInt(item) });
		}
    ...
    app.post("/api/offer", [jsonParser, csrfProtection, verifyToken], (req, res) => {
    	i18n.setLocale("en");
    	...
    	const data = schema.validateAsync(req.body).then((data) => {
    		...
    		data = Object.assign(data, { createdAt: curTime });
